	...
	namespace OBJL
	{
			...
			bool LoadFile(std::string Path)
			{
				...
				while (std::getline(file, curline))
				{
					...
					if (algorithm::firstToken(curline) == "f")
					{
						...
						// Add Vertices
						for (UINT i = 0; i < int(vVerts.size()); i++)
						{
						...
						// Add Indices
						for (UINT i = 0; i < int(iIndices.size()); i++)
						{
				...
				for (UINT i = 0; i < MeshMatNames.size(); i++)
				{
		...
		private:
				...
				std::string icurline)
			{
				...
				for (UINT i = 0; i < int(sface.size()); i++)
				{
				...
				if (noNormal)
				{
					...
					for (UINT i = 0; i < int(oVerts.size()); i++)
					{
				...
				const std::vector<Vertex>& iVerts)
			{
				...
				{
					// For every vertex
					for (UINT i = 0; i < int(tVerts.size()); i++)
					{
